...
def plot_udp_ping(logs_ns3_dir, data_out_dir, pdf_out_dir, udp_ping_id, interval_ns):
    local_shell = LocalShell()

    # Check that all plotting files are available
    if not local_shell.file_exists("plot_udp_ping_time_vs_rtt.plt") or \
            not local_shell.file_exists("plot_udp_ping_time_vs_out_of_order.plt"):
        print("The gnuplot files are not present.")
        print("Are you executing this python file inside the plot_udp_ping directory?")
        exit(1)
    ...
    # Read in UDP pings CSV
    udp_pings_csv_columns = read_csv_direct_in_columns(
        logs_ns3_dir + "/udp_pings.csv",
        "pos_int,pos_int,pos_int,int,int,int,int,int,string"
    )
    num_entries = len(udp_pings_csv_columns[0])
    udp_ping_id_list = udp_pings_csv_columns[0]
    ping_no_list = udp_pings_csv_columns[1]
    send_request_timestamp_list = udp_pings_csv_columns[2]
    reply_timestamp_list = udp_pings_csv_columns[3]
    receive_reply_timestamp_list = udp_pings_csv_columns[4]
    latency_to_there_ns_list = udp_pings_csv_columns[5]
    latency_from_there_ns_list = udp_pings_csv_columns[6]
    rtt_ns_list = udp_pings_csv_columns[7]
    arrived_list = udp_pings_csv_columns[8]

    # Find all entries for our pair
    ping_results = []
    prev_ping_no = -1
    for i in range(num_entries):
        if udp_ping_id_list[i] == udp_ping_id:
    ...
    # Data RTT file
    print("TIME VS. RTT")
    data_rtt_filename = "%s/udp_ping_%d_rtt.csv" % (data_out_dir, udp_ping_id)
    with open(data_rtt_filename, "w+") as out_file:
        for val in ping_results:
            if val["is_lost"]:
                out_file.write("%d,%d,0\n" % (udp_ping_id, val["send_request_timestamp"]))
            else:
                out_file.write("%d,%d,%d\n" % (udp_ping_id, val["send_request_timestamp"], val["rtt_ns"]))

    # Show what is produced
    print(" > Data line format..... [udp_ping_id],[send_request_timestamp (ns)],[measured_rtt (ns); 0 if lost]")
    print(" > Produced data file... " + data_rtt_filename)

    # Plot time vs. rtt
    pdf_rtt_filename = "%s/plot_udp_ping_time_vs_rtt_%d.pdf" % (pdf_out_dir, udp_ping_id)
    local_shell.copy_file("plot_udp_ping_time_vs_rtt.plt", "temp.plt")
    ...
    #
    data_out_of_order_filename = "%s/udp_ping_%d_out_of_order_in_intervals.csv" % (data_out_dir, udp_ping_id)
    with open(data_out_of_order_filename, "w+") as f_out:
        for i in range(len(intervals)):
            f_out.write("%d,%.10f,%.10f\n" % (udp_ping_id, intervals[i][0], intervals[i][2]))
            f_out.write("%d,%.10f,%.10f\n" % (udp_ping_id, intervals[i][1] - 0.000001, intervals[i][2]))
    print(" > Calculated in intervals of " + str(interval_ns / 1e6) + " ms")
    print(" > Data line format..... [udp_ping_id],[time moment (ns)],[# of out-of-order]")
    print(" > Produced data file... " + data_out_of_order_filename)

    # Plot time vs. out-of-order
    pdf_out_of_order_filename = "%s/plot_udp_ping_time_vs_out_of_order_%d.pdf" % (pdf_out_dir, udp_ping_id)
    local_shell.copy_file("plot_udp_ping_time_vs_out_of_order.plt", "temp.plt")
