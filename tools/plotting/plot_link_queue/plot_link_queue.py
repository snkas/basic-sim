...
def plot_link_queue(logs_ns3_dir, data_out_dir, pdf_out_dir, from_node_id, to_node_id):
    ...
    queue_pkt_time_ns_list = queue_pkt_csv_columns[2]
    queue_pkt_size_pkt_list = queue_pkt_csv_columns[3]
    ...
    queue_byte_time_ns_list = queue_byte_csv_columns[2]
    queue_byte_size_byte_list = queue_byte_csv_columns[3]

    # For packets
    data_filename = "%s/link_queue_%d_to_%d_pkt.csv" % (data_out_dir, from_node_id, to_node_id)
    with open(data_filename, "w+") as f_out:
        for i in range(queue_pkt_num_entries):
            if queue_pkt_from_list[i] == from_node_id and queue_pkt_to_list[i] == to_node_id:
                f_out.write("%f,%f\n" % (queue_pkt_time_ns_list[i], queue_pkt_size_pkt_list[i]))
    ...
    # For byte
    data_filename = "%s/link_queue_%d_to_%d_byte.csv" % (data_out_dir, from_node_id, to_node_id)
    with open(data_filename, "w+") as f_out:
        for i in range(queue_byte_num_entries):
            if queue_byte_from_list[i] == from_node_id and queue_byte_to_list[i] == to_node_id:
                f_out.write("%f,%f\n" % (queue_byte_time_ns_list[i], queue_byte_size_byte_list[i]))
