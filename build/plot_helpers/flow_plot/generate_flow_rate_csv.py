...
def generate_flow_rate_csv(logs_ns3_dir, data_out_dir, flow_id, interval_ns):
    ...
    # Now go over the intervals, and write them as
    #
    ...
    with open(data_filename, "w+") as f_out:
        for i in range(len(intervals)):
            ...
            f_out.write("%d,%f,%f\n" % (flow_id, intervals[i][0], rate_megabit_per_s))
            f_out.write("%d,%f,%f\n" % (flow_id, intervals[i][1] - 0.000001, rate_megabit_per_s))
...
if __name__ == "__main__":
    ...
    if len(args) != 4:
        ...
        print("Usage: python generate_flow_rate_csv.py [logs_ns3_dir] [data_out_dir] [flow_id] [interval in ns (int)]")
        exit(1)
    else:
        generate_flow_rate_csv(
            args[0],
            args[1],
            int(args[2]),
            int(args[3])
